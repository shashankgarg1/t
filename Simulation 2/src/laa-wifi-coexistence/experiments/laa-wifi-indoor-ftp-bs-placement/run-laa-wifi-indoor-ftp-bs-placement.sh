     ...
     for transport in ${transports} ; do
      for energyDetection in ${energyDetectionList} ; do
       for cwUpdateRule in ${cwUpdateRules} ; do
        ...
        ./waf --run laa-wifi-indoor --command="%s --cellConfigA=${cell} --cellConfigB=Wifi --lteDutyCycle=${lteDutyCycle} --lbtTxop=${lbtTxop} --logPhyArrivals=${logPhyArrivals} --logCwChanges=${logCwChanges} --logBackoffChanges=${logBackoffChanges} --logBeaconArrivals=${logBeaconArrivals} --logBeaconNodeId=${logBeaconNodeId} --logWifiRetries=${logWifiRetries} --logWifiFailRetries=${logWifiFailRetries} --logHarqFeedback=${logHarqFeedback} --logTxops=${logTxops} --transport=${transport} --duration=${duration} --simTag=${simTag} --outputDir=${outputDir} --RngRun=${RngRun}  --wifiStandard=${wifiStandard} --laaEdThreshold=${energyDetection} --ftpLambda=${ftpLambda} --cwUpdateRule=${cwUpdateRule} --drsPeriod=${drsPeriod} --dropPackets=${dropPackets} --tcpRlcMode=${tcpRlcMode} --rlcAmRbsTimer=${rlcAmRbsTimer} --voiceEnabled=${voiceEnabled} --ns3::TcpSocket::SegmentSize=${tcpSegSize} --ns3::TcpSocket::InitialCwnd=${tcpInitialCw} --wifiQueueMaxSize=2000 --ns3::LteEnbRrc::DefaultTransmissionMode=2 --ns3::SpectrumWifiPhy::CcaMode1Threshold=${wifiEd} --bsCornerPlacement=${bsCornerPlacement}"
       done
      done
     done
    done       
