...
public:
   ...
   */
  std::vector<uint32_t> GetOperationalChannelList () const;
  ...
  void CreateWifiSpectrumPhyInterface (Ptr<Node> node, Ptr<NetDevice> device);
  ...
  */
  ...
  Ptr<SpectrumPhy> GetSpectrumPhy (void) const;
   ...
   */
  enum WifiPhy::ChannelModel GetChannelModel (void) const;
...
private:
  SpectrumWifiPhy (SpectrumWifiPhy const &);
  SpectrumWifiPhy& operator= (SpectrumWifiPhy const &);
  ...
  /**
   * Set the channel model used for frame synchronization error rates.
   *
   * \param model the channel model
   */
  void SetFrameSyncErrorRateChannelModel (enum WifiPhy::ChannelModel model);
  ...
  enum WifiPhy::ChannelModel m_channelModel; //!< Channel model used for error rates
