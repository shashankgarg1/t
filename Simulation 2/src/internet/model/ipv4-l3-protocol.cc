                          ...
                          const Address &to, NetDevice::PacketType packetType)
{
  ...
  // remove tag that may not have been stripped when packet left another node
                      ...
                      Ptr<Ipv4Route> route)
{
  ...
  if (found)
    {
      tos = ipTosTag.GetTos ();
      NS_LOG_DEBUG ("Found ToS " << (uint16_t) tos);
    }
                             ...
                             Ipv4Header const &ipHeader)
{
  ...
  // add class of service tag for lower device layers
  ClassOfServiceTag cosTag;
  // In some tunneling situations, tag may already have been added
  packet->RemovePacketTag (cosTag);
  cosTag.SetClassOfService (ipHeader.DscpTypeToClassOfService (ipHeader.GetDscp ())); 
  NS_LOG_DEBUG ("Set class of service to " << (uint16_t) ipHeader.DscpTypeToClassOfService (ipHeader.GetDscp ()));
