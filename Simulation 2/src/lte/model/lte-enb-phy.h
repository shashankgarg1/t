...
public:
  ...
  */
  ...
  /*
   * \param set if reservation signal should be used when used channel access manager
   */
...
Time GetGrantTimeout() const;
  ...
  */
  ...
  //void TransmitPartialSubFrame (void);
   ...
   *  \brief Duration of grant for tranmission.
  */
  ...
  */
  ...
  typedef void (* ReportTxopTracedCallback)
    ...
    */
   void SetChannelAccessManager (Ptr<ChannelAccessManager> channelAccessManager);

   /*
    * Returns a pointer to the channel access manager.
    */
...
private:
  ...
  /*
   * \brief Can be used to send a channel reservation signal when there is no data ready to be sent.
   */
  ...
  */
  ...
  /*
   * Reservation signal trace source. Reports that reservation signal started and it reports its start time and duration.
   */
  ...
  /*
   * Transmission opportunity trace source. Reports that transmission opportunity started and it reports its start time and duration. It also provide start time of next subframe.
   */
  ...
  /*
   * Important to be able to update  m_currentSrsOffset which is used for UL CQI reporting
   * and for obtaining correct RNTI.
   */
  ...
  /*
   * Variable that indicates if the channel access request was sent and the lte-enb-phy is waiting for channel access grant.
   * If is false, means that we lte-enb-phy is not waiting for channel, so it is ok to request channel access.
   */
  bool m_isWaitingForChannelAccessGrant;

  /*
   * Indicates whether a reservation signal shall be sent when there is no data ready to be transmitted.
   */
  ...
  uint32_t m_mibPeriod;
  uint32_t m_sibPeriod;
  uint32_t m_drsPeriod;
  bool m_drsMessagesEnabled;
  Time m_disableMibAndSibStartupTime;
  bool m_generateCtrlAndRbStats;
  bool m_channelAccessImplWith2msDelay;
  bool m_dropPackets;
  std::map<Time, double> m_logTimeToRbUsage;
  std::map<int,int> m_ctrlTypesCount;
  int m_ctrMsgCounter;
  Time m_lastDrsMessageSent;
  Time m_channelAccessManagerStartTime;
