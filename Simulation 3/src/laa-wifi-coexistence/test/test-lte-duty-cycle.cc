...
LteDutyCycleTestCase::DoRun (void)
{
  ...
  Simulator::Schedule (MilliSeconds (0), &LteDutyCycleTestCase::DelayedDutyCycleStart, this, enbDevs);
                                     ...
                                     uint8_t mcsTb1, uint16_t sizeTb1, uint8_t mcsTb2, uint16_t sizeTb2)
{
  ...
  if (Simulator::Now () > MilliSeconds (10))
  {
  ...
  // do not consider first 8ms of each grant period, until MCS gets its real value. This is becaue there might not
  // be transmission because of delay between MAC and PHY for first 2 ms of each grant period, so inferference will
  // not be present. While MCS is not updated there is delay of 4ms, and 2 ms while this MCS is actual
  if (Simulator::Now () > MilliSeconds (300) && tInt>8)
    {
