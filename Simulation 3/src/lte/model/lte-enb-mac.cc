...
LteEnbMac::DoSubframeIndication (uint32_t frameNo, uint32_t subframeNo)
{
  ...
  {
  	std::cout<<"lte-enb-mac\n";
	  m_dlHarqFeedback(m_dlInfoListReceived);
  }
 ...
 /* if (m_ulCeReceived.size () > 0)
  ...
  m_ulSchedFrameNo = m_frameNo;
  m_ulSchedSubframeNo = m_subframeNo;
  //   NS_LOG_DEBUG (this << " sfn " << frameNo << " sbfn " << subframeNo);
  ...
  for (  uint32_t i  = 0; i < ind.m_buildDataList.size (); i++ )
    {
      ...
      if (ind.m_buildDataList.at (i).m_dci.m_tbsSize.size () == 1)
        {
          m_dlScheduling (m_dlSchedFrameNo, m_dlSchedSubframeNo, ind.m_buildDataList.at (i).m_dci.m_rnti,
...
//std::vector<DlInfoListElement_s> vec;
//  vec.push_back(params);
//  m_dlHarqFeedback(vec);
}
