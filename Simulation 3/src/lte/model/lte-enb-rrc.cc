...
LteEnbRrc::ConfigureCell (uint8_t ulBandwidth, uint8_t dlBandwidth,
                          uint32_t ulEarfcn, uint32_t dlEarfcn, uint16_t cellId)
{
...
{
 NS_LOG_FUNCTION (this << absPattern);
 ...
 NS_ASSERT_MSG (absPattern[15] == 0 || absPattern[35] == 0,
                ...
                "Watch out for the bit order in the std::bitset constructor from string."); 
 m_cmacSapProvider->SetAbsPattern (absPattern);
 m_cphySapProvider->SetAbsPattern (absPattern);
}
