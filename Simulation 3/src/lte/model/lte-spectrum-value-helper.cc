...
/**
 * This table provides information about the E-UTRA Absolute Radio Frequency Channel Number and the corresponding carrier frequency values for different frequency bands.
 *
 * The values for the bands 1-40 have been obtained from:
 ...
 *
 * The values for the bands 252-255 have been obtained from:
 * LTE-U Technical Report, "Coexistence Study for LTE-U SDL", V1.0 (2015-02)
 */
...
LteSpectrumValueHelper::GetSpectrumModel (uint32_t earfcn, uint8_t txBandwidthConfiguration)
{
  ...
  if (it != g_lteSpectrumModelMap.end ())
  ...
  else
    {
      ...
      NS_LOG_LOGIC ("creating SpectrumModel for EARFCN=" << earfcn << " fc=" << fc << " bw=" << (uint32_t) txBandwidthConfiguration);
