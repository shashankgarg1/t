...
bool
PfSchedulerMemberCschedSapProvider::IsThereData (void)
{
                      ...
                      for (uint8_t k = 0; k < nLayer; k++)
                        {
                          ...
                          if (itMap != allocationMap.end ())
                            {
                              tbSize = m_amc->GetTbSizeFromMcs (mcs, rbgSize * (itMap->second.size() + 1));
                            }
                          else
                            {
                              tbSize = m_amc->GetTbSizeFromMcs (mcs, rbgSize);
                            }
      ...
      if ((*it).first.m_rnti == rnti)
        {
          NS_LOG_INFO ("subframe: " << Simulator::Now().As (Time::MS) << " rnti:"<< (*it).first.m_rnti << "\t"
              <<(*it).second.m_rlcTransmissionQueueSize << "\t" << (*it).second.m_rlcRetransmissionQueueSize << "\t" << (*it).second.m_rlcStatusPduSize);
          lcCounter++;
