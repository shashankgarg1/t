...
LteEnbPhy::GetTypeId (void)
{
  ...
  .AddAttribute ("SibPeriod",
       	         ...
       	         UintegerValue (20),  /// \todo In what unit is this?
  ...
  .AddAttribute ("DrsPeriod",
                 ...
                 UintegerValue (40),  /// \todo In what unit is this?
  ...
  .AddAttribute("ImplWith2msDelay",
                "It is ment to be used when channel access manager is set;"
...
LteEnbPhy::DoDispose ()
{
  ...
  if (m_generateCtrlAndRbStats)
      ...
      if (!outFileCtrlMessages.is_open ())
	{
      ...
      outFileCtrlMessages << "Number of subframes used to send all CTRL messages:"<<m_ctrMsgCounter<< " "<< std::endl;
...
{
  NS_LOG_FUNCTION (this << m_txPower);
...
LteEnbPhy::StartSubFrame (void)
{
  ...
  // piece of code that needs to be executed each subframe, no mather if we are are transmiting subframe or not
   ...
   */
  if (m_disableMibAndSibStartupTime==Seconds(0) || ( m_disableMibAndSibStartupTime!=Seconds(0) &&  Simulator::Now() <= m_disableMibAndSibStartupTime))
    {
  ...
  if (m_channelAccessManager == 0 || (m_channelAccessManager!=0 && Simulator::Now() < m_channelAccessManagerStartTime) )
    {
      TransmitSubFrame();
    }
  else  // if there is channel access manager
    {
      if (m_channelAccessImplWith2msDelay)
        ...
        {
          // if there is no data neither ctrl messages ready to be transmitted shift the queues
          if (!IsNonEmptyCtrMessage() && !IsNonEmptyPacketBurst())
            {
              GetControlMessages ();
              GetPacketBurst();
          ...
          /*else if (IsNonEmptyCtrMessage() && !IsNonEmptyPacketBurst()) // if there is only CTRL and no data, just transmit even if there is no grant
	      	{
	      	  TransmitSubFrame();
	      	}*/
          /*else if (drsMessageReady && ((m_ttiBegin - m_lastDrsMessageSent) >= m_drsPeriod))  // there is drs message ready to be sent
	       {
		  TransmitSubFrame();
		  //if we want to implement to send DRS without LBT when period expires,
		  // then we need to save other ctrl messages so that they can be send with data when txop is approved
	      }*/
          else
            {
              ...
              // channel access not granted
              else
                {
                  ...
                  if (!m_isWaitingForChannelAccessGrant && (IsNonEmptyPacketBurst() || IsNonEmptyCtrMessage()))
                    {
                      RequestChannelAccess ();
                    }
                }
                  ...
                  if(m_dropPackets)
                    {
                      ...
                      if ((m_dropPackets) && ((IsNonEmptyPacketBurst() && m_hasUlDci)||m_hasUlDci))
                        {
                          NS_LOG_DEBUG("Dropping PDU ");
                          GetControlMessages();
                          GetPacketBurst();
                        }
...
LteEnbPhy::TransmitSubFrame (void)
{
  ...
  if (isAlmostBlankSubframe) // Almost Blank Subframe
	...
	if (m_channelAccessManager && (Simulator::Now() > m_channelAccessManagerStartTime)) // check if to send reservation signal after ctrl
	  {
	    // in the case of drs messages we send reservation signal to simulate that drs message lasts 1 subframe instead of only 3 symbols
	    if (m_channelAccessImplWith2msDelay)
	      {
		...
		else //there is nothing to transmit, release the grant after ctrl messages are sent
		  {
...
/*
void
LteEnbPhy::TransmitPartialSubFrame (void)
{
  NS_LOG_FUNCTION (this);
  Time reservationSignalStartTime;

  if (m_grantTimeout >= DL_CTRL_DELAY_FROM_SUBFRAME_START)
    {
	  std::list<Ptr<LteControlMessage> > ctrlMsg = GetControlMessages ();
      SendControlChannels (ctrlMsg);
      Simulator::Schedule (DL_CTRL_DELAY_FROM_SUBFRAME_START, // ctrl frame fixed to 3 symbols
...
LteEnbPhy::DoSendReservationSignal ()
{
  ...
  //NS_ASSERT_MSG(m_reservationSignal, "When m_reservationSignal is false, DoSendReservationSignal function should not be called.");
  NS_ASSERT_MSG(m_channelAccessManager && (Simulator::Now()>=m_channelAccessManagerStartTime), "When m_channelAccessManager is not set, SendReservationSingal function should not be called.");
...
void
LteEnbPhy::SetChannelAccessManager (Ptr<ChannelAccessManager> channelAccessManager)
{
	NS_LOG_FUNCTION (this);
