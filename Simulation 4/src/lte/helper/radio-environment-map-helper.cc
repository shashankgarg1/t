  ...
  static TypeId tid = TypeId ("ns3::RadioEnvironmentMapHelper")
   ...
   .AddAttribute ("XRes", "The resolution (number of points) of the map along the x axis.",
    ...
    .AddAttribute ("NoiseFigureDb",
                   "the noise figure of the measuring instrument noise in dB, relative to a kT of -174 dBm.",
                   DoubleValue (9),
                   MakeDoubleAccessor (&RadioEnvironmentMapHelper::m_noiseFigureDb),
                   MakeDoubleChecker<double> ())
...
RadioEnvironmentMapHelper::DelayedInstall ()
{
  ...
  Ptr<SpectrumModel> sm = LteSpectrumValueHelper::GetSpectrumModel (m_earfcn, m_bandwidth);

  double noisePowerSpectralDensity = LteSpectrumValueHelper::GetNoisePowerSpectralDensity (m_noiseFigureDb);

  if (m_rbId >= 0)
    {
      m_noisePowerW = noisePowerSpectralDensity * 180000;
    }
  else
    {
      m_noisePowerW = noisePowerSpectralDensity * 180000 * m_bandwidth;
    }
