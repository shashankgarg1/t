...
void
LbtAccessManagerBaseTestCase::SpectrumWifiPhyReceiver (bool rxSucceeded)
{
  m_count++;
}
...
LbtAccessManagerBaseTestCase::DoSetup (void)
{
  ...
  m_phy->SetPacketReceivedCallback (MakeCallback (&LbtAccessManagerBaseTestCase::SpectrumWifiPhyReceiver, this));
...
LbtTransmitImmediatelyAfterRequest::DoRun (void)
{
  ...
  //Signal lasts for 1292 microseconds, then we wait for 43 microsecond defer
  //Should be able to send immediately at time 7001335 but not before
  //
  //Simulator::Schedule (MicroSeconds (7001335), &LbtAccessManagerBaseTestCase::RequestAccess, this);
  Simulator::Schedule (MicroSeconds (7001335), &LbtAccessManagerBaseTestCase::RequestAccess, this);
  ...
  NS_TEST_ASSERT_MSG_EQ (m_accessGrantedTimes[1].GetMicroSeconds (), 7001335, "Access provided too early or late");
}
...
LbtDeferAndBackoff::DoRun (void)
{
  ...
  // So check that access granted is at 6000000 + 1292 + 43 + 11*9 = 6001434
  ...
  // The LBT should be busy until 7001292, then defer for 43, then backoff
  // But interrupt the defer at 7001300 with another signal, which holds it busy
  // again until 7002592
  Simulator::Schedule (MicroSeconds (7001300),  &LbtAccessManagerBaseTestCase::SendSignal, this, txPowerWatts); 
  ...
  int64_t expectedExpiration0 = 6000000 + 1292 + 43 + 9 * m_backoff[0];
  int64_t expectedExpiration1 = 7001300 + 1292 + 43 + 9 * m_backoff[1];
...
LbtSuspendBackoff::DoRun (void)
{
  ...
  // The LBT should be busy until 8001292, then defer for 43, then backoff
  // starts at 6001335
  // Backoff in this case is 11 slots
  ...
  Simulator::Schedule (MicroSeconds (8001350),  &LbtAccessManagerBaseTestCase::SendSignal, this, txPowerWatts); 
  // Store the value of backoff counter (should be decremented by two)
  Simulator::Schedule (MicroSeconds (8001351), &LbtSuspendBackoff::GetBackoff, this);
  ...
  int64_t expectedExpiration0 = 8000000 + 1350 + 1292 + 43 + 9 * m_backoff[1];
