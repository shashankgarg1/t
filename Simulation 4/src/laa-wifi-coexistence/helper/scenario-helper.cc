...
static ns3::GlobalValue g_logHareFeedbackNodeId ("logHarqFeedbackNodeId",
...
class CwStatistics: public Object
{

public:

	uint32_t countIncreased;
	uint32_t countReset;
	uint32_t maxCwSize;
	...
	void CwIncreased(uint32_t oldValue, uint32_t newValue);
	void CwReset(uint32_t oldValue, uint32_t newValue);
};

// TBD count statistics
void CwStatistics::CwIncreased(uint32_t oldValue, uint32_t newValue)
{
	countIncreased++;
	if (newValue > maxCwSize)
		maxCwSize = newValue;
}

void CwStatistics::CwReset(uint32_t oldValue, uint32_t newValue)
{
	countReset++;
}
...
/*
void
ConfigureWifiCwCallback(Ptr<NetDevice> wifiNetDevice, Ptr<CwStatistics> cwStatistics)
{
  Ptr<WifiMac> wifiMac = wifiNetDevice->GetObject<WifiNetDevice> ()->GetMac ();
  Ptr<RegularWifiMac> regularWifiMac = DynamicCast<RegularWifiMac> (wifiMac);

  NS_ASSERT_MSG(regularWifiMac!=0, "RegularWifiMac does not exist.");
  NS_ASSERT_MSG(regularWifiMac->GetDcfManager() != 0, "DcfManager does not exist.");
  std::vector<DcfState *> dcfStates =  regularWifiMac->GetDcfManager()->GetStates();
  // we should have at least one DcfState per wifi device
  NS_ASSERT_MSG(dcfStates.size()>0, "No dcf states for contention window statistics.");

  for (uint32_t i = 0; i < dcfStates.size() ;  i++)
  {
	  dcfStates.at(i)->GetDcfCwTrace()->TraceConnectWithoutContext("DcfCWIncrease", MakeCallback(&CwStatistics::CwIncreased,cwStatistics));
	  dcfStates.at(i)->GetDcfCwTrace()->TraceConnectWithoutContext("DcfCWReset", MakeCallback(&CwStatistics::CwReset,cwStatistics));
  }
}
*/
...
{
  Config::Connect ("/NodeList/*/DeviceList/*/$ns3::WifiNetDevice/Mac/$ns3::ApWifiMac/BE_EdcaTxopN/$ns3::EdcaTxopN/BackoffTrace", MakeCallback (&BackoffChangeCb));
  Config::Connect ("/NodeList/*/DeviceList/*/$ns3::WifiNetDevice/Mac/$ns3::StaWifiMac/BE_EdcaTxopN/$ns3::EdcaTxopN/BackoffTrace", MakeCallback (&BackoffChangeCb));
}

void
ScheduleWifiBackoffLogDisconnect (void)
{
  Config::Disconnect ("/NodeList/*/DeviceList/*/$ns3::WifiNetDevice/Mac/$ns3::ApWifiMac/BE_EdcaTxopN/$ns3::EdcaTxopN/BackoffTrace", MakeCallback (&BackoffChangeCb));
  Config::Disconnect ("/NodeList/*/DeviceList/*/$ns3::WifiNetDevice/Mac/$ns3::StaWifiMac/BE_EdcaTxopN/$ns3::EdcaTxopN/BackoffTrace", MakeCallback (&BackoffChangeCb));
}

void
ScheduleCwChangesLogConnect (void)
{
  Config::Connect ("/NodeList/*/DeviceList/*/$ns3::WifiNetDevice/Mac/$ns3::ApWifiMac/BE_EdcaTxopN/$ns3::EdcaTxopN/CwTrace", MakeCallback (&CwChangeCb));
  Config::Connect ("/NodeList/*/DeviceList/*/$ns3::WifiNetDevice/Mac/$ns3::StaWifiMac/BE_EdcaTxopN/$ns3::EdcaTxopN/CwTrace", MakeCallback (&CwChangeCb));
}

void
ScheduleCwChangesLogDisconnect (void)
{
  Config::Disconnect ("/NodeList/*/DeviceList/*/$ns3::WifiNetDevice/Mac/$ns3::ApWifiMac/BE_EdcaTxopN/$ns3::EdcaTxopN/CwTrace", MakeCallback (&CwChangeCb));
  Config::Disconnect ("/NodeList/*/DeviceList/*/$ns3::WifiNetDevice/Mac/$ns3::StaWifiMac/BE_EdcaTxopN/$ns3::EdcaTxopN/CwTrace", MakeCallback (&CwChangeCb));
}
...
ConfigureWifiAp (NodeContainer bsNodes, struct PhyParams phyParams, Ptr<SpectrumChannel> channel, Ssid ssid)
{
  ...
  GlobalValue::GetValueByName ("wifiQueueMaxSize", uValue);
  Config::SetDefault ("ns3::WifiMacQueue::MaxPacketNumber", UintegerValue (uValue.Get ()));
  ...
  EnumValue enumValue;
  GlobalValue::GetValueByName ("wifiStandard", enumValue);
  if (enumValue.Get () == St80211a)
    {
  ...
  else if (enumValue.Get () == St80211n)
    {
  ...
  else if (enumValue.Get () == St80211nFull)
    {
      wifi.SetStandard (WIFI_PHY_STANDARD_80211n_5GHZ);
      WifiMacHelper mac;
...
void
PrintCwStatsToFile(Ptr<CwStatistics> cwStatistics, Ptr<CwStatistics> lteCwStatistics, std::string outFileName)
{
  IntegerValue integerValue;
  double lbtTxop = 0;
  double rngRun = 0;

  if (GlobalValue::GetValueByNameFailSafe("lbtTxop", integerValue))
    {
	  lbtTxop = integerValue.Get ();
    }

  if ( GlobalValue::GetValueByNameFailSafe ("RngRun", integerValue))
    {
	  rngRun = integerValue.Get ();
    }

  std::ofstream cwStatsFile;  ///< Stream the output to a file.
  std::string cwUpdateOutFile = outFileName + "_CW_update_stats";

  cwStatsFile.open (cwUpdateOutFile.c_str (), std::ofstream::app);
  if (!cwStatsFile.is_open ())
    {
      NS_FATAL_ERROR ("Can't open file " << (cwUpdateOutFile));
      return;
    }

  cwStatsFile << lbtTxop  << "\t"
  			<< rngRun  << "\t"
                 << cwStatistics->countIncreased << "\t"
                 << cwStatistics->countReset << "\t"
                 << cwStatistics->maxCwSize << "\t"
                 << lteCwStatistics->countIncreased << "\t"
                 << lteCwStatistics->countReset << "\t"
                 << lteCwStatistics->maxCwSize << std::endl;
                         ...
                         std::string simulationParams)
{
 ...
 Ptr<CwStatistics> cwStatistics = CreateObject<CwStatistics>();
 Ptr<CwStatistics> lteCwStatistics = CreateObject<CwStatistics>();
  ...
  if (cellConfigA == WIFI)
    {
      ...
      /*
      if (generateCwStatistics)
        {
    	  NS_ASSERT_MSG(ueDevicesA.GetN()>0, "CW statitics cannot be configured, no device.");
          Ptr<NetDevice> netDevice = ueDevicesA.Get(0);
          // Start to measure statistics once that everything is installed and applications are started
          Simulator::Schedule(serverStartTime, &ConfigureWifiCwCallback, netDevice, cwStatistics);
        }
        */
  ...
  if (disableApps == false)
    {
      ...
      if (transport == UDP)
      ...
      else if (transport == FTP)
        {
          ...
          std::fstream outfile;
          outfile.open("/home/shashank/Workspace/ns-3-lbt-ce3ba0988994/src/updateCW.dat",std::ios::out|std::ios::trunc);
          outfile<<1;
          outfile.close();
  ...
  if (generateCwStatistics)
    {
        PrintCwStatsToFile (cwStatistics, lteCwStatistics, outFileName);
    }
    */
  //
  ...
  std::cout << "--- List of associations (time and node id) ---" << std::endl;
  for (std::vector<AssociationEvent>::size_type i = 0; i < g_associations.size (); i++)
    {
      std::cout << std::setprecision (6) << std::fixed << g_associations[i].m_time.GetSeconds () << " " << g_associations[i].m_nodeId << " " << g_associations[i].m_type << " " << g_associations[i].m_address << std::endl;
    }
