...
InterferenceHelper::AddForeignSignal (Time duration, double rxPowerW)
{
  ...
  Add (0, fakeTxVector, fakePreamble, duration, rxPowerW);
}
...
InterferenceHelper::CalculateChunkSuccessRate (double snir, Time duration, WifiMode mode, WifiTxVector txVector) const
{
  ...
  else // using an AWGN error rate model
    {
  ...
  if (nbits == 0)
    {
      nbits = 1; // XXX above arithmetic may lead to zero value
    }
...
InterferenceHelper::CalculateAvgSyncFieldSnir (Ptr<InterferenceHelper::Event> event)
{
  ...
  Time plcpSyncFieldEnd = packetStartTime + WifiPhy::GetPlcpPreambleDuration (txVector, preamble); // packet start time + sync field
