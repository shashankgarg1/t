...
PacketSocket::Connect (const Address &ad)
{
  ...
  if (m_state == STATE_CLOSED)
    {
      NS_LOG_DEBUG ("PacketSocket Connect() error: BADF");
      m_errno = ERROR_BADF;
      goto error;
    }
  if (m_state == STATE_OPEN)
    {
      ...
      NS_LOG_DEBUG ("PacketSocket Connect() error: INVAL");
      m_errno = ERROR_INVAL; // generic error condition.
      goto error;
    }
  if (m_state == STATE_CONNECTED)
    {
      NS_LOG_DEBUG ("PacketSocket Connect() error: ISCONN");
      m_errno = ERROR_ISCONN;
      goto error;
    }
  if (!PacketSocketAddress::IsMatchingType (ad))
    {
      NS_LOG_DEBUG ("PacketSocket Connect() error: AFNOSUPPORT");
...
PacketSocket::Send (Ptr<Packet> p, uint32_t flags)
{
  ...
  if (m_state == STATE_OPEN ||
      m_state == STATE_BOUND)
    {
      NS_LOG_DEBUG ("Error:  not connected");
