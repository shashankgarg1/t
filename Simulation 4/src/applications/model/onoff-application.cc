...
OnOffApplication::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::OnOffApplication")
    ...
    .AddAttribute ("Protocol", "The type of protocol to use.",
                   ...
                   MakeTypeIdChecker ())
    .AddAttribute ("TypeOfService", "Socket IP_TOS value to use",
                   UintegerValue (0),
                   MakeUintegerAccessor (&OnOffApplication::SetTypeOfService,
                                         &OnOffApplication::GetTypeOfService),
...
uint32_t
OnOffApplication::GetTotalBytes (void) const
{
  NS_LOG_FUNCTION (this);
  return m_totBytes;
}
  ...
  if (!m_socket)
    {
      ...
      if (GetTypeOfService ())
        {
          NS_LOG_DEBUG ("Setting Type of Service to " << (uint16_t) GetTypeOfService ());
          m_socket->SetIpTos (GetTypeOfService ());
        }
...
void
OnOffApplication::SetTypeOfService (uint8_t tos)
{
...
uint8_t
OnOffApplication::GetTypeOfService (void) const
{
  NS_LOG_FUNCTION (this);
